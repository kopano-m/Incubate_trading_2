...
from TSDualMA import TradingStrategyDualMA
...
def load_financial_data(start_date, end_date, output_file):
    try:
        ...
        print('File data found...reading EURUSD data')
    except FileNotFoundError:
        print('File not found...downloading the EURUSD data')
        df = data.DataReader('EURUSD=X', 'yahoo', start_date, end_date)
...
for line in zip(EURUSD_data.index, EURUSD_data['Adj Close']):
    date = line[0]
    price = line[1]
...
plt.plot(eb.ts.list_paper_total,
         label="Paper Trading using Event-Based BackTester")
